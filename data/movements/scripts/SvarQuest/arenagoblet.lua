function onStepIn(cid, item, position, fromPosition) 
	...
	elseif item.actionid == 42380 then 
      ...
      elseif item.actionid == 42390 then 
		if getPlayerStorageValue(cid, 42390) ~= 1 then 
			setPlayerStorageValue(cid, 42390, 1) 
			local goblet = doCreateItemEx(10137, 1) 
			doItemSetAttribute(goblet, "description", "It is given to the courageous victor of the barbarian arena demonic difficulty.\nAwarded to " .. getCreatureName(cid) .. ".") 
			doTileAddItemEx({x=gobletPos.x,y=gobletPos.y-1,z=gobletPos.z}, goblet) 
		end 
     end
